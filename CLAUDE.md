...
## v3 Simplification Note

Issue `#75` 以降の新規作業では、次を user-facing default として扱う。

- 主語は `PR -> test understanding -> manual test design -> GitHub QA issue handoff`
- coarse-grained surface は `analyze-pr` / `design-handoff` / `publish-handoff`
- GitHub Issue が primary artifact
- `config.json` / local DB / progress files は current implementation では依然使うが、新規設計では optional cache / resume layer として扱う
- 11-step workflow は internal / legacy surface であり、通常利用の主語にしない
- 旧 skill を単純削除するより、近い責務を public skill にマージして残した skill の役割を広げる
- public skill は順番にリレーし、各 skill の中で不足文脈を質問してよい
