...
user-facing の flow は 3 つの public skill で完結する。

1. `analyze-pr` — PR / related issue / changed files / existing tests / layer applicability を整理する
...
- CLI は fact collection / rendering / publish の実装層、意味づけと次 step への確認は skill 側が持つ
...
Skill Layer (skills/*/SKILL.md)
  │  各スキルは「ステートレス関数」: CLI で事実を集め → 整理 → GitHub Issue に publish
...
|---|---|
| ワークフロー制御・対話 | PR/MR 取得・diff 解析 |
| AskUserQuestion で不足文脈を補完 | test mapping・risk analysis |
| CLI 呼び出し・結果要約 | allocation・handoff markdown 生成 |
| 次スキルへの誘導 | GitHub Issue publish |
...
- `src/exploratory-testing/models/` — Valibot スキーマと型定義（config, pr-intake, test-mapping, risk-assessment, allocation, github-issue など）
- `src/exploratory-testing/db/` — SQLite スキーマと repository（`bun:sqlite` 使用）
- `src/exploratory-testing/tools/` — CLI サブコマンドの実装（analyze-pr, design-handoff, publish-handoff, doctor, config, manifest, および内部 tool 関数）
- `src/exploratory-testing/analysis/` — 解析ロジック（change analysis, test discovery, risk scoring など）
- `src/exploratory-testing/scm/` — SCM 抽象（GitHub / GitLab provider）
- `skills/` — 各 public skill の SKILL.md（Claude Code Plugin として読まれる）
...
- `doctor` / `db init` / `manifest show` の CLI が存在
- `analyze-pr` / `design-handoff` / `publish-handoff` の 3-skill public flow が実装済み
...
- テスト surface は 3-skill public flow に合わせて簡素化済み
...
CLI (`cli/index.ts`) に provider 判定や fetch ロジックを直接書かない。CLI の責務は引数受け取り + JSON 出力のみ。ビジネスロジックは `tools/` に、SCM 抽象は `scm/` に置く。
...
`savePrIntake` のように INSERT → 直後の SELECT で結果を返すパターンは `database.transaction()` で包む。
...
同様に、stability notes や「既存テストの注意点」を user-facing output に出すときも、
候補全体ではなく `coverageGapMap.coveredBy` に出現する asset のみを対象にすること。
候補パスに `flaky/` が含まれていても、それが実在するか coverage に寄与しているかは別の話。
実在しない候補を「既存テストの注意点」として出すと QA に誤情報を渡す。
...
- 分類根拠は change analysis の `fileAnalyses[].categories` から引く
...
数回の workspace 初期化、DB 書き込みを含むテストは 5 秒を超えることがある。
...
このリポジトリの CLI は後続 skill から機械的に読まれる前提。  
...
### Skill は独自 markdown を組み立てない

publish-handoff の body は必ず CLI の `renderHandoffMarkdownV2()` が生成する。  
skill 側で handoff markdown を手書き・独自構成して `handoff create-issue --body` や `gh issue create` で publish しない。

- `handoff create-issue --body` / `handoff update-issue --body` は内部低レベル API であり、skill からは使わない
- skill が CLI を呼ばず独自に body を作ると、renderer version marker が付かず、形式の一貫性が崩れる
- draft の要約はしてよいが、publish する body は `publish-handoff --pr <number>` 経由の CLI 出力をそのまま使う
...
handoff 本文では file path や reason を Markdown table に流し込むことが多い。  
`|` を含む path や説明文をそのまま出すと table が壊れる。

- table cell に入れる path / reason / coveredBy は `escapePipe()` 相当で処理する
- free-text 入力は改行も `<br>` に置換すること。`|` だけ escape しても `\n` が table 行を分断する

GitHub Issue に出力される Markdown を壊さないこと。
...
- 後続処理は基本的に「最新 intake にぶら下がる analysis/mapping」を扱う
