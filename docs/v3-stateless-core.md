...
- 各 skill の途中で、CLI や PR / issue から取れない不足文脈だけをユーザーに確認する
- 各 skill の終わりで成果を要約し、次の skill に進んでよいか確認する
...
- 質問は、CLI や PR / issue / diff から取れない文脈に限る
- 各 public skill の終わりで、次の skill に進んでよいかを確認する
...
- それでも不足する intent context があればユーザーに質問する
- 整理結果を要約する
...
- それでも不足する運用判断やスコープがあればユーザーに質問する
- handoff draft を要約する
...
- skill contract 上は config が空の項目だけをユーザーに確認する
