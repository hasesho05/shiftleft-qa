...
- 先に CLI から draft 作成に必要な事実を集め、それでも不足する運用判断があればユーザーに質問する。

## 実行手順

1. この PR の analysis をもとに、`already covered` / `should automate` / `manual exploration required` を整理する。
2. 現在の実装で必要なら、内部的に `allocate` と `handoff generate` を使って draft を作る。
3. ambiguity や scope の判断が残る場合は、ユーザーに不足文脈を確認する。
4. handoff draft を要約する。
5. `publish-handoff` に進んでよいか確認する。

現在の実装で使う CLI 例:

- `bun run dev allocate run --risk-assessment-id <id>`
- `bun run dev allocate summary --risk-assessment-id <id>`
- `bun run dev handoff generate --risk-assessment-id <id>`
...
- handoff draft の要約後、`publish-handoff` に進んでよいかを確認する。

例:

- 「handoff draft は以上です。この内容で `publish-handoff` に進めてよければ続けます。」
