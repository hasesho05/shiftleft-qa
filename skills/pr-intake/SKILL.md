...
> Legacy internal step. 通常利用では `analyze-pr` の内部処理として吸収される想定。

## 目的

変更要求を永続化された成果物として保存する。

user-facing には「PR を取り込む step」ではなく、「analysis の内部で metadata と intent context を集める処理」として扱う。
