...
import {
  type HandoffViewModel,
  buildHandoffViewModel,
} from "../analysis/build-requirement-view";
import {
...
export async function generateHandoffMarkdown(
  ...
  const viewModel = buildHandoffViewModel({
    intentContext: context.intentContext,
    allocationItems: context.items,
    testMapping: context.testMapping,
    changeAnalysis: context.changeAnalysis,
    prIntake: context.prIntake,
  });

  const prUrl =
    context.prIntake.provider === "github"
      ? `https://github.com/${context.prIntake.repository}/pull/${context.prIntake.prNumber}`
      : `${context.prIntake.repository}#${context.prIntake.prNumber}`;
  ...
  return {
    ...
    markdown: renderHandoffMarkdownV2(viewModel, prUrl, input.riskAssessmentId),
    sections,
...
/** @deprecated Use renderHandoffMarkdownV2 instead. */
...
export function renderHandoffMarkdownV2(
  ...
  const lines: string[] = [
    `<!-- rendererVersion: ${RENDERER_VERSION} -->`,
    `## QA Handoff — ${escapePipe(viewModel.title)}`,
    "",
  ];

  // Requirements section
  lines.push("### 実装要件", "");
  if (viewModel.requirements.length === 0) {
    lines.push("_要件情報なし_", "");
  } else {
    for (const req of viewModel.requirements) {
      lines.push(`- ${escapePipe(req.requirement)}`);
      if (req.relatedTests.length > 0) {
        lines.push(
          `  - 関連テスト: ${req.relatedTests.map((t) => `\`${escapePipe(t)}\``).join(", ")}`,
        );
      }
      if (req.sourceFiles.length > 0) {
        lines.push(
          `  - 根拠ソース: ${req.sourceFiles.map((s) => `\`${escapePipe(s)}\``).join(", ")}`,
        );
      }
      if (req.automationCandidates.length > 0) {
        lines.push(
          `  - 自動テスト候補: ${req.automationCandidates.map((c) => `\`${escapePipe(c)}\``).join(", ")}`,
        );
      }
    }
    lines.push("");
  ...
  // Test layers section
  lines.push("### テストレイヤー", "");
  if (viewModel.testLayers.length === 0) {
    lines.push("_該当なし_", "");
  } else {
    for (const layer of viewModel.testLayers) {
      lines.push(`- ${layer.label}`);
      for (const testPath of layer.evidence) {
        lines.push(`  - \`${escapePipe(testPath)}\``);
      }
    }
    lines.push("");
  }
  ...
  lines.push("### 手動確認が必要な項目", "");
  if (viewModel.manualChecks.length === 0) {
    lines.push("_手動確認不要_", "");
  } else {
    for (const check of viewModel.manualChecks) {
      lines.push(
        `- ${escapePipe(check.description)} — ${escapePipe(check.reason)}`,
      );
    }
    lines.push("");
  }

  // Notes section (always rendered)
  lines.push("### 備考", "");
  ...
  // Footer
  lines.push("---", "");
  lines.push(`<!-- riskAssessmentId: ${riskAssessmentId} -->`);
  lines.push(`- PR: ${prUrl}`);
  lines.push(`- Generated by shiftleft-qa ${RENDERER_VERSION}`);
  lines.push("");
