...
describe("handoff tool", () => {
  ...
  it("generates markdown from allocation items with escaped markdown-sensitive content", async () => {
    ...
    // v2 format: requirement-based sections
    expect(result.markdown).toContain("## QA Handoff");
    expect(result.markdown).toContain("### 実装要件");
    expect(result.markdown).toContain("### テストレイヤー");
    expect(result.markdown).toContain("### 手動確認が必要な項目");
  });
  ...
  it("publishes a generated handoff issue via the gh wrapper", async () => {
    ...
    expect(vi.mocked(createIssue)).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        repository: "owner/repo",
        labels: ["qa-handoff"],
        body: expect.stringContaining("QA Handoff"),
      }),
    );
  ...
  it("renders confidence bucket and hypothesis disclaimer in handoff markdown", async () => {
    ...
    // v2 format: no confidence badges or old section headings
    expect(result.markdown).toContain("## QA Handoff");
    expect(result.markdown).toContain("### 実装要件");
    expect(result.markdown).toContain("Generated by shiftleft-qa");
  });
  ...
  it("renders layer applicability notes so non-primary layers are not mistaken for missing work", async () => {
    ...
    // v2 format: simplified test layers instead of layer applicability
    expect(result.markdown).toContain("### テストレイヤー");
    expect(result.markdown).toContain("ビジュアルテスト");
  });
